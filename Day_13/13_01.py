...
* Floor starts at (0,0) (upper left)
* Lobby is at (1,1)
* NEED TO READ UP ON BREADTH FIRST SEARCH!!
"""
...
def is_wall( x, y ):
	"""
	# True if wall, False if floor
	"""

	val = x * x + 3 * x + 2 * x * y + y + y * y + _INPUT
	num_ones = bin( val ).count( '1' )
	return num_ones % 2 == 1


def _draw_office( columns, rows ):
	"""
	Debug function to visualize maze
	"""

	for i in range( rows ):
		r = ''
		for j in range( columns ):
			char = "#" if is_wall( j, i ) else "."
			r += char
