...
is dropped and tries to fall through slots in a set of rotating disks to finally 
...
The disks pause their motion each second and come in different sizes; they seem 
...
Furthermore, the disks are spaced out so that after you push the button, one 
...
position 0). In this case, the capsule would fall through the disks and come out 
of the machine.

However, your situation has more than two disks; you've noted their positions in 
your puzzle input. What is the first time you can press the button to get 
a capsule?

PERSONAL NOTES:
* Given a list of each of the positions on each disk The positions for each disk 
  for the solution can be precalculated by the following:
  
  for x in range( len( disks ):
     solution[ x ] = ( disks[ x ] - ( x + 1 ) ) % disks[ x ]
"""
