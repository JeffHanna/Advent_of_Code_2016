...
* At each loop iteration the direction (L or R) will need to be converted to
  a new vector (-X, +X, -Y, or +Y) depending on the current facing. Probably
  easiest to do this with a lookup mapping. In Python 3.x an Enum class
  could be used.	  
'''

import os
puzzle_input_filepath = os.path.abspath( os.path.join( os.getcwd( ), '01_01_puzzle_input.txt' ) )
